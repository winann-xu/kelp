...
状态：**部分完成 —— hub 已部署并常驻，但外部可达性被阿里云安全组阻塞**（2026-09-19 22:0x~22:2x）
...
- [ ] 用户放行安全组 11010（TCP+UDP）；如需面板，另加面板端口（预留 18080/TCP）
- [ ] 放行后复测：50.9 与 Mac 通过 `tcp://47.116.73.216:11010` 接入 → `easytier-cli peer` 双向可见
